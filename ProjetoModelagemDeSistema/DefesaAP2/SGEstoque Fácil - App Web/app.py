...
@app.route('/excluirProduto/<int:idProduto>', methods=['DELETE'])
def excluir_produto(idProduto):
    ...
    return {"status": "success", "message": "Produto excluído com sucesso!"}, 200
...
@app.route('/excluirTipo/<int:idTipoProduto>', methods=['DELETE'])
def excluir_tipo(idTipoProduto):
    ...
    if count > 0:
        conn.close()
        return {"status": "error", "message": "Não é possível excluir este tipo de produto, pois existem produtos associados a ele."}, 400
    else:
        ...
        return {"status": "success", "message": "Tipo de produto excluído com sucesso!"}, 200
